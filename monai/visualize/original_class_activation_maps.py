...
__all__ = ["originalCAM", "originalGradCAM", "originalGradCAMpp", "originalModelWithHooks", "originaldefault_normalizer"]


def originaldefault_normalizer(x: NdarrayTensor) -> NdarrayTensor:
    """
...
class originalModelWithHooks:
    ...
    def forward_hook(self, name):
        def _hook(_module, _input, output):
            ...
            self.activations[name][0].detach()
    ...
    def __call__(self, x, class_idx=None, retain_graph=False):
        ...
        self.model.eval()
        logits = self.model(x)
        #make_dot(self.model(x), params=dict(self.model.named_parameters()), show_attrs=True, show_saved=True)
        ...
        if self.register_backward:
            ...
            self.score.sum().backward(retain_graph=True)
            for layer in self.target_layers:
                if layer not in self.gradients:
                    warnings.warn(
                        f"Backward hook for {layer} is not triggered; `requires_grad` of {layer} should be `True`."
                    )           
            #local_grad = tuple(torch.autograd.grad(self.score.mean(), x, create_graph=True, retain_graph=True)[0] for layer in self.target_layers if layer in self.gradients)
            #make_dot(grad, params=dict(list(self.model.named_parameters()) + [('x', x)])).render("attached", format="png")
...
class originalCAMBase:
    ...
    def __init__(
        ...
        postprocessing: Callable = originaldefault_normalizer,
        register_backward: bool = True,
    ) -> None:
        # Convert to model with hooks if necessary
        if not isinstance(nn_module, originalModelWithHooks):
            self.nn_module = originalModelWithHooks(
...
class originalCAM(originalCAMBase):
    """
...
class originalGradCAM(originalCAMBase):
    """
...
class originalGradCAMpp(originalGradCAM):
    """
    Computes Gradient-weighted Class Activation Mapping (Grad-CAM++).
    This implementation is based on:
